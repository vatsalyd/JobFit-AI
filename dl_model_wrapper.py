...
class DLModelWrapper:
    def __init__(self, model_dir, model_name="sentence-transformers/all-MiniLM-L6-v2", device=None):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = ResumeMatchModel(model_name).to(self.device)
        self.model.load_state_dict(torch.load(f"{model_dir}/dl_resume_match.pt", map_location=self.device))
        self.model.eval()
